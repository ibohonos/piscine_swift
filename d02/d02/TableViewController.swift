...
class TableViewController: UITableViewController {
    ...
    @IBAction func unwindFromAddVC(_ sender: UIStoryboardSegue) {
        if sender.source is ViewController {
            if let senderVC = sender.source as? ViewController {
                if senderVC.name != "" {
                    Data.death.append((senderVC.name, senderVC.desc, senderVC.date))
                }
            }
...
//        cell.detailTextLabel?.text = Data.death[indexPath.row].1
        ...
        deathTable.estimatedRowHeight = 44.0
